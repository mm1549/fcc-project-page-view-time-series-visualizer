...
def draw_bar_plot():
    ...
    df_bar = df.groupby(pd.Grouper(freq='M')).sum()
    df_bar['Years'] = df_bar.index.strftime('%Y')
    ...
    sns.barplot(data=df_bar, x='Years', y='value', hue='Months', hue_order = hue_order, ax=ax, errorbar=None, palette='Set2')
